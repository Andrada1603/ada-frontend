...
const PlayerLessonsTable = () => {
  ...
  const renderLesson = ({ _id, name, coach, date, location, sport }, index) => {
    const handleClick = () => router.push('./lessons/schedule');
    return (
      <tr key={`area-${name}`}>
        <td>{index + 1}</td>
        <td>
        ...
        <td onClick={() => handleClick(_id)}>{coach.last_name}, {coach.first_name} </td>
  ...
  return (
    <div>
      ...
      <main className="max w-full lg:col-span-5 p-4 lg:p-8 xl:px-12">
          <h4 className='mb-4'> Au fost gasite {data?.pageParams.count} lecții la care vă puteți programa </h4>
          <div>
      <table>
        <thead>
          <th>#</th>
